...
dp = 1;   
WP_SC_type  = 101;      % full_LTS = 100 - full_HTS = 101 - Hybrid = 102
...
TitleName = sprintf('TF FUTURE A%G WP %s Optimum TCM.xlsx',A(dp)*10,WP_SC_type);
%% Ammissibili acciaio
...
Iop_max = 5.5e4;  % Operative current - max
...
%% solo LTS
% if max(check_type_HTS) > 0 && min(WP_SC_type == 'full_LTS') ~= 0 continue 
% end
