...
%% Storage name
TitleName = sprintf('TF WP - Rect - CEFTR - %s.xlsx',today("datetime"));
...
Iop_min = 3.0e4;  % Operative current - min
Ntlmax = ceil(NI/Iop_min); 
Ntlmin = ceil(NI/Iop_max);
min_size_CICC = 0.03; % min CICC size
max_size_CICC = 0.08; % max CICC size
min_JT = 0.002; 
...
%%          
            if B_layers(1,1)> 15
                [~, HF] = min(abs(B_layers - B_layers(1,1)));
                [~, MF] = min(abs(B_layers - 13));
                [~, LF] = min(abs(B_layers - 6.5));
                n_grades = [HF MF LF];
            elseif B_layers(1,1)> 10
                [~, HF] = min(abs(B_layers - B_layers(1,1)));
                [~, MF] = min(abs(B_layers - 9));
                [~, LF] = min(abs(B_layers - 6));
                n_grades = [HF MF LF];
            else
                n_grades = 1;
            end
            ...
            for var = n_grades
                B = B_layers(1,var);       
                [type_cable(1,var),N_Cu(1,var),N_Sc(1,var),N_tot(1,var),S_Cable(1,var),S_REBCO(1,var),S_Cu_HTS(1,var),THS(1,var),mat(1,var)] ...
...
%% Cechck geometrico sulle dimensioni ottenute nel cavo
            ...
            if min(SC_w) <= 0.005 || min(r_cable(1:n_layers))< 0.99 || max(r_cable(1:n_layers))>2.5 || min(JT(1:n_layers)) < min_JT
                continue
            end
...
%% Primary radial stress (Pm+Pb)
            ...
            % scf_jacket = scf(param, type_cable, var, E_cbl_LTS, E_cbl_HTS)     
...
%%
            if S_T_VT < S_amm_JT*1.2 && S_T_JT < S_amm_JT*1.2 && S_T_JT>0 && S_T_VT>0 
                counter =  counter+1;  
                n_cond =  n_spire_(1,1);
                WP_w = WP_w0(1,1);
                ...
                DATA(counter,:) = table(dp,S_T_VT,S_T_JT,R_0,B_PHI_0,B_TF,Iop,JENG,L,E,Ri_,Rj_,Rk_,radial_build,Nose,WP_h,WP_w,...
                    lateral_w,GoundIns,n_cond,n_grades,n_layers,n_turns,type_cable,Cond_w,Cond_h,JT,r_cable,N_Sc,N_Cu,S_Cable,S_REBCO,S_Cu_HTS,THS,Tau_discharge,mat);
