...
TitleName = sprintf('TF DEMO A%G - PROTO.xlsx',A(dp)*10);
%% Ammissibili acciaio
...
dr_plasma_side  = 0.060; % [m] Spessore case fronte plasma
toroidal_gap    = 0.025; 
GoundIns        = 0.010; % Ground insulation WP
...
Iop_max = 8.5e4;  % Operative current - max
...
%% Preallocations
            ...
            S_CICC = zeros(1,n_layers);
            S_JT = zeros(1,n_layers);
            Ri = zeros(1,n_layers);
            Re = zeros(1,n_layers);
...
%% Induttanza TF modello shell
            ...
            k_bf = 0.5*log(R_TF_Outerleg/R_TF_Innerleg);  
