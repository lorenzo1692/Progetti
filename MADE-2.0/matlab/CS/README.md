...
Scaffolding per la pipeline di design del Central Solenoid (CS), speculare
alla pipeline del TF (vedi `main_WP_TF_design.m` e le cartelle `input/`,
`io/`, `physics/`, `search/`, `postprocess/` in `MADE-2.0/matlab/`):

- `input/` — file di input (es. template Excel) con i parametri del CS.
