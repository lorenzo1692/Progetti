...
%%
% % % R0   = [7.5 6.5 7.4 9.0 3.2 8.9380 2.55 2.5 7.8];              % [m]
% % % B0   = [4.0 6.6 11.4 5.4 7.4 4.89 6.0 7.0 3.9];                % [T]
% % % A    = [2.6 3.3 4.5 3 6.0 3.1 5.3 5.5 2.6.];
% % % RTFi = [3.02 2.9 4.178 4.3 1.77 4.6248 1.22 1.255 3.4];        % [m]
% % % RTFo = [13.837 11.9 12.310 12.0 5.26  16.2756 4.456 4.39 13.25]; % [m]
...
R_VV = RTFi(dp)*1.05;                  % [m]
%% Storage name 
TitleName = sprintf('TF DEMO A%G WP %s.xlsx',A(dp)*10,WP_SC_type);
%% Ammissibili acciaio
S_amm_JT = (667*1e6);
S_amm_VT = (667*1e6);
S_amm_Cu = (100*1e6);
...
TitleName = sprintf('Iop vs Rk - A%G WP %s',A(dp)*10,WP_SC_type);
...
TitleName = sprintf('Rk vs Jeng - A%G WP %s',A(dp)*10,WP_SC_type);
title(TitleName,'fontsize',20,'Interpreter','latex')
grid on
print('-dpng','-r300',TitleName);
%%
% figure('units','normalized','outerposition',[0.25 0.1 0.5 0.9])
% Ii = table2array((DATA(1:end,6)));
% U = table2array((DATA(1:end,31)));
% Bb = table2array((DATA(1:end,5)));
% Rkk = table2array((DATA(1:end,9)));
% Sint = table2array((DATA(1:end,3)));
% NIi = table2array((DATA(1:end,6))).*table2array((DATA(1:end,15)))*1e-6;
% 
% scatter(U,Rkk,20,Sint,'filled') 
% colormap(jet);
% set(gca,'FontSize',20);
% colorbar
% c = colorbar;
...
% mod_max = round(max(Sint));
% mod_min = round(min(Sint));
% caxis([mod_min mod_max]);
% MAXMIN=get(c,'Limits');
% T = (linspace(MAXMIN(1),MAXMIN(2),8));
% set(c,'Ticks',T)
% ylabel('Rk [m]','fontsize',20,'Interpreter','latex')
% xlabel('U [$MJ$]','fontsize',20,'Interpreter','latex')
% TitleName = sprintf('Rk vs U - A%G WP %s',A(dp)*10,WP_SC_type);
% title(TitleName,'fontsize',20,'Interpreter','latex')
% grid on
% print('-dpng','-r300',TitleName);
