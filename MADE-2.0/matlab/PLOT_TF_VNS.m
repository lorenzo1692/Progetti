...
shape_cable = 201;      % 200 = RIS - 201 = Rect
...
n_layers = 12; 
maxdim = n_layers;
% n_turns = ones(1,n_layers)*[10]; 
n_turns = [10 10 10 10 8 8 8 8 8 8 6 6];
n_grades = 3;
grading_at = [1 5 11 (n_layers+1)]; %% il grade viene applicato a partire dal layer indicato (incluso)
...
for g = 1:n_grades
    var =  grading_at(g);
    B = B_layers(1,var);       
    [type_cable(1,var),N_Cu(1,var),N_Sc(1,var),N_tot(1,var),S_Cable(1,var),S_REBCO(1,var),S_Cu_HTS(1,var),THS(1,var),mat,Ic_sc] ...
...
for var = 1:n_layers 
    ...
    % grading jacket
    % idx = find(var <= flip(grading_at(1:end-1)), 1, 'last');
    % 
    % if isempty(idx)
    %     p_rs = B_TF^2/(2*Mu_0);   
    % else
    %     p_rs = B_layers(grading_at(idx))^2/(2*Mu_0);   
    % end
...
TitleName2 = sprintf('n grades=%G, N=%G , Iop=%G kA, Sa(VT)=%G MPa, Sa(JT)=%G MPa, tau=%G s.png',n_grades,n_spire_(1), round(Iop*1e-3),round(S_amm_VT*1e-6/safety_membrane),round(S_amm_JT*1e-6/safety_membrane),Tau_discharge);
...
text(0,Ri_*1.05,sprintf('WP(w)=%G m',round(WP_w,3)),'HorizontalAlignment','center','fontsize',12,'Interpreter','latex')
...
xlabel('[m]','fontsize',20,'Interpreter','latex')
ylabel('[m]','fontsize',20,'Interpreter','latex')
TitleName1 = sprintf('n grades=%G, N=%G , Iop=%G kA, Sa(VT)=%G MPa, Sa(JT)=%G MPa, tau=%G s .png',n_grades,n_spire_(1), round(Iop*1e-3),round(S_amm_VT*1e-6/safety_membrane),round(S_amm_JT*1e-6/safety_membrane),Tau_discharge);
...
xlabel('[m]','fontsize',20,'Interpreter','latex')
ylabel('[m]','fontsize',20,'Interpreter','latex')
TitleName2 = sprintf('WP n grades=%G, N=%G , Iop=%G kA, Sa(VT)=%G MPa, Sa(JT)=%G MPa, tau=%G s .png',n_grades,n_spire_(1), round(Iop*1e-3),round(S_amm_VT*1e-6/safety_membrane),round(S_amm_JT*1e-6/safety_membrane),Tau_discharge);
title(TitleName2,'fontsize',20,'Interpreter','latex');
