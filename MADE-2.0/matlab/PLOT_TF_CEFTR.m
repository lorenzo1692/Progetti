...
n_layers = 9;  
maxdim = n_layers;
% n_turns = ones(1,n_layers)*[18]; 
n_turns = [18 18 18 18 16 16 16 16 16];
n_grades = 2;
grading_at = [1 5 (n_layers+1)]; %% il grade viene applicato a partire dal layer indicato (incluso)
% grading_at = [1 (n_layers+1)];
%%
lateral_w = 0.145;
min_JT = 0.002; 
V_MAX = 10e3; % [V]
...
S_amm_JT = (1500*1e6)/SF_pm/SF_oop;
S_amm_VT = (1400*1e6)/SF_pm/SF_oop;
S_amm_Cu = (100*1e6);
...
ins_grades      = 0.001; % Spessore isolante tra i Grades
...
Ke_cavo_rad = zeros(1,n_layers); 
Ke_cavo_tor = zeros(1,n_layers); 
...
%%
for g = 1:n_grades
    var =  grading_at(g); % B_layers(1,grading_at(1:end-1))
    B = B_layers(1,var);       
    [type_cable(1,var),N_Cu(1,var),N_Sc(1,var),N_tot(1,var),S_Cable(1,var),S_REBCO(1,var),S_Cu_HTS(1,var),THS(1,var),mat] ...
    = CICC_CEFTR(B,Iop,Tau_discharge,WP_SC_type); 
...
S_z_JT =  T_bf/(WP_w0(1,1)^2)/1;

p_rs = B_TF^2/(2*Mu_0);  

% for g = 1:n_grades
%     varg =  grading_at(g);
%     B_flip = flip(B_layers);
%     p_rs = (B_flip(1,varg))^2/(2*Mu_0);                                 % Magnetic pressure, thin WP
% 
%     for var = grading_at(g):grading_at(g+1)-1      
    ...
    for var = 1:n_layers     
        ...
        elseif shape_cable == 201  
            ...
            while S_rm_grades(1,var) > S_amm_JT
...
while S_T_VT > S_amm_VT*1.2 || S_c_VT > S_amm_VT || S_T_JT > S_amm_JT*1.2 || S_rm_JT > S_amm_JT
...
% fill([-WP_w_tot/2 -WP_w_tot/2 WP_w_tot/2 WP_w_tot/2 -WP_w_tot/2],[Rj_-GoundIns Ri_-dr_plasma_side Ri_-dr_plasma_side Rj_-GoundIns Rj_-GoundIns],'m'), hold on
...
for var = 1:n_layers
    for t = 1:n_turns(var)                                
        ...
        hold on                     
        if shape_cable == 200                  
            rectangle('Position',[(Cond_w(1,var)*(-1-n_turns(var)/2)+Cond_w(1,var)/2-SC_w(1,var)/2)+Cond_w(1,var)*t Ri(1,var)+JT(1,var)+3*tins(1,var)-2*ins_grades SC_w(1,var) SC_w(1,var)],...
...
MAXMIN=get(c,'Limits');
T = round(linspace(MAXMIN(1),MAXMIN(2),sum(n_layers)),2);
set(c,'Ticks',T)
...
plot([-dx_ -dx_],[Rj_ (Rj_+WP_h_tot)],'--.k','linewidth',1); hold on;  
plot([-dx_ -dx_],[Rk_ Rj_],'--.k','linewidth',1); hold on; 
plot([-dx_ -dx_],[(Rj_+WP_h_tot) Ri_],'--.k','linewidth',1); hold on; 
plot([-WP_w_tot/2 WP_w_tot/2],[Ri_*1.03 Ri_*1.03],'--.k','linewidth',1); hold on;  
plot([-CASE_w/2 CASE_w/2],[Ri_*1.08 Ri_*1.08],'--.k','linewidth',1); hold on; 
dx_ = -CASE_w;
text(dx_,Rk_,sprintf('Rk=%G m',round(Rk_,3)),'fontsize',12,'Interpreter','latex')
text(dx_,Rj_,sprintf('Rj=%G m',round(Rj_,3)),'fontsize',12,'Interpreter','latex')
text(dx_,Ri_,sprintf('Ri=%G m',round(Ri_,3)),'fontsize',12,'Interpreter','latex')
text(-dx_*1.03,Rj_+WP_h_tot/2,sprintf('WP(h)=%G m',round(WP_h_tot,3)),'fontsize',12,'Interpreter','latex')
text(-dx_*1.03,(Rk_+Rj_)/2,sprintf('Nose(h)=%G m',round(Rj_-Rk_,3)),'fontsize',12,'Interpreter','latex')
text(-dx_*1.03,((Rj_+WP_h_tot)+Ri_)/2,sprintf('dps(h)=%G m',round(dr_plasma_side,3)),'fontsize',12,'Interpreter','latex')
text(0,Ri_*1.1,sprintf('Case(w)=%G m',round(CASE_w,3)),'HorizontalAlignment','center','fontsize',12,'Interpreter','latex')
text(0,Ri_*1.05,sprintf('WP(w)=%G m',round(WP_w_tot,3)),'HorizontalAlignment','center','fontsize',12,'Interpreter','latex')
...
for var = 1:n_layers
    for t = 1:n_turns(var)                                
        ...
        if shape_cable == 200                  
            ...
            'FaceColor',[0.9290 0.6940 0.1250],'Curvature',[1 1])          
        else
            rectangle('Position',[(-Cond_w(1,var)-Cond_w(1,var)*n_turns(var)/2+JT(1,var)+tins(1,var))+Cond_w(1,var)*t Ri(1,var)+JT(1,var)+tins(1,var) Cond_w(1,var)-2*JT(1,var)-2*tins(1,var) Cond_h(1,var)-2*JT(1,var)-2*tins(1,var)],...
            'FaceColor',[0.9290 0.6940 0.1250],'Curvature',[0 0])
        end         
    end
end
axis equal
...
xlabel('[m]','fontsize',20,'Interpreter','latex')
ylabel('[m]','fontsize',20,'Interpreter','latex')
TitleName2 = sprintf('WP n grades=%G, N=%G , Iop=%G kA, Sa(VT)=%G MPa, Sa(JT)=%G MPa, tau=%G s',n_grades,n_spire_(1), round(Iop*1e-3),round(S_amm_VT*1e-6),round(S_amm_JT*1e-6),round(Tau_discharge));
title(TitleName2,'fontsize',20,'Interpreter','latex');
% ylim([Rj_*0.9 Ri_*1.1])
% xlim([-WP_w WP_w])
if printcond == 1, print('-dpng','-r300',TitleName2), end
...
% % % 
% % % mod_max = 15.5;
% % % map = jet(ceil(mod_max));
% % % B_max_layers = linspace(0,mod_max,ceil(mod_max));
% % % % Trova il valore più vicino
% % % [~, idx] = min(abs(B_max_layers - B_PHI_TF));
% % % colorvar = floor(linspace(1,idx,n_layers));
% % % 
% % % fill([-CASE_w_l/2 -CASE_w/2 CASE_w/2 CASE_w_l/2 -CASE_w_l/2],[Rk_ Ri_ Ri_ Rk_ Rk_],[0.7 0.7 0.7]), hold on
% % % for var = 1:n_layers  
% % %     for t = 1:n_turns(var)                           
...
% % %         else
% % %             rectangle('Position',[(-Cond_w(1,var)-Cond_w(1,var)*n_turns(var)/2+JT(1,var)+tins(1,var))+Cond_w(1,var)*t Ri(1,var)+JT(1,var)+tins(1,var) Cond_w(1,var)-2*JT(1,var)-2*tins(1,var) Cond_h(1,var)-2*JT(1,var)-2*tins(1,var)],...
% % %             'FaceColor',map(colorvar(n_layers-var+1),:),'Curvature',[0 0])
% % %         end          
% % %     end
% % % end
% % % set(gca,'FontSize',20);
